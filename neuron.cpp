...
template<NType>
	...
	Neuron::Neuron(const unsigned int &previousSize, const unsigned int &forwardSize, std::vector<NType> *weightData = NULL, NType bias = -1)
	{
	...
	const std::vector<NType> &Neuron::getWeights()
	{
		return weights;
	}

	NType Neuron::getBias()
	{
		return bias;
	}


	void Neuron::updateBias(const NType &bias)
	{
		this->bias -= bias;
	}

	void Neuron::updateWeights(const std::vector<NType> &weightDeltas)
	{
